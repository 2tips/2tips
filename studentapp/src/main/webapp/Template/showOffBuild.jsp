...
<head>
    ...
    <script type="text/javascript">
        $(function () {
            $('.test').click(function () {
                alert("订单号"+$('#offId').text()+$("#tempStatus").find("option:selected").text())
                window.location.href = '/Template/changeOffStatus?errId='+$('#offId').text()+'&status='+$("#tempStatus").find("option:selected").text();
            });
        })
    </script>
</head>
...
<table class="tbform">
    <form action="/Template/findOffBuild" method="post">
        <thead>
        <tr>
...
<table class="tb" id="list">
    ...
    <c:forEach var="offBuildes" items="${offBuildes}">
        <tr>
            <td>${offBuildes.id}</td>
            <td id="offId">${offBuildes.offId}</td>
            <td>${offBuildes.status}</td>
            <td>
