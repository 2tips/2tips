...
<head>
    ...
    <script type="text/javascript">
        ...
        //     });
        // })
        function modify(errId) {
            var v1 = $('#errAppId'+errId).text();
            var tempStatus = $("#tempStatus"+errId).find("option:selected").text();
            alert(v1 + tempStatus)
            // $("#status"+accId).text(tempStatus);
            // alert("申请,"+$('#accAppId'+accId).text()+$("#"+("tempStatus"+accId)).find("option:selected").text())
            window.location.href = '/apply/examErrApply?errId='+ v1 +'&status='+ tempStatus;
        }
    </script>
</head>
<body>
...
<table class="tbform">
    <form action="/apply/findErrApply" method="post">
        <thead>
        <tr>
...
<table class="tb" id="list">
    ...
    <c:forEach var="applies" items="${applies}">
        <tr>
            <td id=errAppId${applies.errId}>${applies.errId}</td>
            <td>${applies.userId}</td>
            <td>${applies.name}</td>
            <td>${applies.sex}</td>
            <td>${applies.address}</td>
            <td>${applies.phone}</td>
            <td>${applies.errCause}</td>
            <td id=status${applies.errId}>${applies.status}</td>
            <td>
                <select id=tempStatus${applies.errId}>
                    <option>审核通过</option>
                    <option>审核不通过</option>
                </select>
                <input type="button" class="accapp" value="修改" onclick="modify(${applies.errId}">
            </td>
        </tr>
    </c:forEach>
</table>
